...
const renderCreator = () => {
  const card = el('form', 'admin-card founder-creator'); card.noValidate = true
  ...
  card.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      await queue(job.id, card, manuscriptPath, coverPath); setStatus(status, 'Queued. The external worker will detect, import, generate, and validate this candidate.', 'success'); await load()
...
const renderCatalog = () => {
  ...
  rows.forEach((item) => {
    const row = el('form', 'admin-platform-item'); const title = el('strong', '', `${item.type}: ${item.title}`); const price = document.createElement('input'); price.type = 'number'; price.min = '1'; price.value = ((Number(item.price_amount) || 0) / 100).toFixed(2)
...
const renderPasses = () => {
  ...
  const form = el('form', 'admin-access-grid'); const title = field('title', 'Title', 'text', '', true); const slug = field('slug', 'Slug', 'text', '', true); const price = field('price', 'Price (₹)', 'number', '99', true); const duration = field('duration', 'Duration hours', 'number', '24', true); const scope = select('scope', 'Scope', [{ id: 'collection', title: 'Collection' }, { id: 'library', title: 'Library' }], 'collection'); const collection = select('collection', 'Collection', state.collections); const active = select('active', 'Status', [{ id: 'true', title: 'Active' }, { id: 'false', title: 'Inactive' }], 'true'); const save = el('button', 'admin-action admin-action--primary', 'Save 1-Day Pass'); save.type = 'submit'
  form.append(title, slug, price, duration, scope, collection, active, save); card.append(form)
  form.addEventListener('submit', async (event) => { event.preventDefault(); if (!form.reportValidity()) return; try { const library = form.elements.scope.value === 'library'; const payload = { title: form.elements.title.value.trim(), slug: form.elements.slug.value.trim().toLowerCase(), price_amount: paise(form.elements.price.value), duration_hours: Number(form.elements.duration.value), scope_type: library ? 'library' : 'collection', collection_id: library ? null : form.elements.collection.value || null, active: form.elements.active.value === 'true' }; if (!library && !payload.collection_id) throw new Error('Choose a collection-scoped pass or select Library.'); const { error } = await supabase.from('temporary_access_passes').upsert(payload, { onConflict: 'slug' }); if (error) throw error; await load() } catch (error) { alert(error.message) } })
  if (!state.passes.length) card.append(el('p', 'admin-empty', 'No access passes are configured.'))
  state.passes.forEach((pass) => card.append(el('p', '', `${pass.active ? 'Active' : 'Inactive'} · ${pass.title} · ${rupees(pass.price_amount)} · ${pass.duration_hours} hours`)))
  return card
}

const render = () => { if (!host) return; host.replaceChildren(renderCreator(), renderJobs(), renderCatalog(), renderPasses()) }
const start = async () => {
  if (!host) return
  ...
  state.profile = profile; await load()
  window.clearInterval(timer); timer = window.setInterval(() => { if (document.querySelector('[data-admin-panel="publishing"]')?.classList.contains('is-active')) load().catch(() => {}) }, POLL_MS)
}
start().catch(() => {})
